...
class ImageEncoder(nn.Module):
    ...
    def forward(self, x):
        """
        x: 4D-Tensor (batch_size=1, channel, height, width)
        """
...
class ImageDecoder(nn.Module):
    ...
    def forward(self, zm, zc):
        """
        zm: 2D-Tensor (batch_size, dim_zm)
        zc: 2D-Tensor (batch_size, dim_zc)
        """
        z = torch.cat([zm, zc], dim=1) # z: (batch_size, dim_z)
        z = z.reshape(z.size(0), self.dim_z, 1, 1) # z: (batch_size, dim_z, height, width)
        return self.decoder(z) # x_recon: (batch_size, channel, height, width)
